...
import { submitFromBundleWithParams, asyncSubmitApplications } from '../../actions';

export default class SubmissionTimeView {
  constructor(getStore, closePanel, submissionTimeValues, input) {
    ...
    for (let i = 0, item = 0; i < submissionTimeValues.length; i += 1, item += 1) {
      ...
      inputs[item] = (
        <div style={style.inputDivStyle} key={`${compositeName}${name} -- ${i}`}>
  ...
  handleSubmit(event) {
    ...
    } else if (this.input.subType === 'Bundle') {
      const action = submitFromBundleWithParams(this.input.bundleId, this.input.identifier, params);
      this.getStore.dispatch(action);
    } else {
      const action = asyncSubmitApplications(this.input.buildId, true, this.input.identifier, params);
      this.getStore.dispatch(action);
    }
    this.closePanel();
  }
