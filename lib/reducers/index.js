...
const streamsV5Build = (state = [], action) => {
  ...
  switch (action.type) {
    ...
    case actions.PREVIOUS_LOGIN_STEP:
      return {
        ...state,
        currentLoginStep: state.currentLoginStep - 1
      };
    ...
    case actions.AUTHENTICATE_ICP4D:
      return {
        ...state,
        formData: {}, // RESET FORM DATA TO EMPTY
        username: action.username,
        rememberPassword: action.rememberPassword,
    ...
    case actions.GET_BUILD_STATUS_FULFILLED:
      return {
        ...
        builds: {
          ...state.builds,
          [state.selectedInstance.instanceName]: {
            ...state.builds[state.selectedInstance.instanceName],
            [action.buildId]: {
