...
const extractSab = (bundlePath, bundleId) => {
  return new Promise(resolve => {
    ...
    extract(bundlePath, { dir: extractedPath }, (err) => {
      ...
      fs.createReadStream(`${extractedPath}${path.sep}tar${path.sep}bundle.tar`).pipe(tar.extract(`${extractedPath}${path.sep}tar${path.sep}bundle`, {
        finish: () => {
          resolve(getSubmissionTimeParams(bundlePath, extractedPath, bundleId));
        }
      }));
    });
  });
...
const getSubmissionTimeParams = (bundlePath, extractedPath, bundleId) => {
  if (fs.existsSync(`${extractedPath}${path.sep}tar${path.sep}bundle`)) {
    ...
    if (submissionTimeValues) {
      ...
      }, submissionTimeValues.childrenNamed('submissionTimeValue'), null, null, null, bundleId);
    ...
    return from(a.bundles).pipe(
      ...
      mergeMap(bundle => StreamsRestUtils.artifact.uploadApplicationBundleToInstance(s, bundle.bundlePath).pipe(
        tap((uploadRes) => {
          ...
          extractSab(bundle.bundlePath, submitBundleId);
        }),
        delay(5000),
