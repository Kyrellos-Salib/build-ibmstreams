      ...
      mergeMap(artifact => StreamsRestUtils.artifact.getAdl(s, buildId, artifact.id).pipe(
        map((adl) => {
          ...
          if (submissionTimeValues) {
            this.submissionTimeView = new SubmissionTimeView(getStore(), () => {
              this.submissionTimePanel.hide();
            }, submissionTimeValues.childrenNamed('submissionTimeValue'), {
              type: 'V5', subType: 'adl', buildId, identifier: a.identifier
            });
        ...
        map((panel) => {
          if (panel) {
            ...
            return of(submitApplications(buildId, true, []));
          }
        })
      )),
      map(() => ({ type: actions.WAITING_FOR_SUBMISSION_TIME_PARAMETERS })),
      catchError(error => of(handleError(a, error)))
    );
  }),
);
  ...
  mergeMap(([a, s]) => {
    ...
    return from(artifacts).pipe(
      ...
      mergeMap(artifact => StreamsRestUtils.artifact.submitJob(
        ...
        {
          submitParameters: a.submissionTimeValues
        }
      ).pipe(
        tap(submitResponse => {
...
const asyncSubmitApplicationsEpic = (action, state) => action.pipe(
  ofType(actions.ASYNC_SUBMIT_APPLICATIONS),
  withLatestFrom(state),
  mergeMap(([a, s]) => {
    const { buildId } = a;
    let submitInfo;
...
const extractSab = (bundlePath, input) => {
  return new Promise(resolve => {
    ...
    extract(bundlePath, { dir: extractedPath }, (err) => {
      ...
      fs.createReadStream(`${extractedPath}${path.sep}tar${path.sep}bundle.tar`).pipe(tar.extract(`${extractedPath}${path.sep}tar${path.sep}bundle`, {
        finish: () => {
          resolve(getSubmissionTimeParamsFromBundle(bundlePath, extractedPath, input));
        }
      }));
    });
  });
...
const getSubmissionTimeParamsFromBundle = (bundlePath, extractedPath, input) => {
  ...
  mergeMap(([a, s]) => {
    return from(a.bundles).pipe(
      ...
      mergeMap(bundle => StreamsRestUtils.artifact.uploadApplicationBundleToInstance(s, bundle.bundlePath).pipe(
        tap((uploadRes) => {
          const submitBundleId = ResponseSelector.getUploadedBundleId(uploadRes);
          extractSab(bundle.bundlePath, { type: 'V5', subType: 'Bundle', bundleId: submitBundleId });
        }),
        delay(5000),
        mergeMap(uploadBundleResponse => {
          const submitBundleId = ResponseSelector.getUploadedBundleId(uploadBundleResponse);
          if (this.submissionTimePanel) {
            this.submissionTimePanel.show();
            return of({ type: actions.WAITING_FOR_SUBMISSION_TIME_PARAMETERS });
          }
...
const asyncSubmitApplicationsFromBundleFilesEpic = (action, state) => action.pipe(
  ofType(actions.SUBMIT_APPLICATIONS_FROM_BUNDLE_FILES_ASYNC),
  withLatestFrom(state),
  mergeMap(([a, s]) => {
    return from(a.bundles).pipe(
      tap((bundleToUpload) => {
      ...
      mergeMap(bundle => StreamsRestUtils.artifact.uploadApplicationBundleToInstance(s, bundle.bundlePath).pipe(
        ...
        mergeMap(uploadBundleResponse => {
          ...
          return StreamsRestUtils.asyncSubmissions.submitJobAsync(s, submitBundleId, {}).pipe(
            tap(submitResponse => {
              const submitInfo = ResponseSelector.getSubmitInfo(submitResponse);
              StatusUtils.jobSubmitted(s, submitInfo);
            }),
            map((submitResponse) => (getSubmitStatus(ResponseSelector.getSubmitInfo(submitResponse).id, bundle.jobName)))
          );
        })
      )),
      catchError(error => of(handleError(a, error)))
...
const submitApplicationsFromBundleFilesWithParametesEpic = (action, state) => action.pipe(
  ofType(actions.SUBMIT_APPLICATIONS_FROM_BUNDLE_FILES_WITH_PARAMS),
  withLatestFrom(state),
  mergeMap(([a, s]) => StreamsRestUtils.asyncSubmissions.submitJobAsync(s, submitBundleId, { submitParameters: a.submitParameters }).pipe(
    tap(submitResponse => {
      const submitInfo = ResponseSelector.getSubmitInfo(submitResponse);
      StatusUtils.jobSubmitted(s, submitInfo);
    }),
...
const rootEpic = combineEpics(
  ...
  submitApplicationsFromBundleFilesWithParametesEpic,
  ...
  asyncSubmitApplicationsFromBundleFilesEpic,
  asyncSubmitApplicationsEpic
