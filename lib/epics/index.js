...
const submitStatusEpic = (action, state) => action.pipe(
  ...
  mergeMap(([a, s]) => zip(
    StreamsRestUtils.asyncSubmissions.getJobSubmission(s, a.id).pipe(
      map(response => getSubmitStatusFulfilled(ResponseSelector.getSubmitStatus(response))),
      catchError(error => of(handleError(a, error)))
    ),
  ...
  // emit [BUILD_STATUS_FULFILLED, BUILD_LOG_FULFILLED, BUILD_STATUS_RECEIVED]
  mergeMap(([statusFulfilledAction, logFulfilledAction]) => [statusFulfilledAction, logFulfilledAction, submitStatusReceived(statusFulfilledAction.id)])
);
...
const shouldGetSubmitStatusHelperObservable = (action, state) => {
  ...
  if (submitStatus === 'submission.unknown' || submitStatus === 'submission.failedProcessingJob' || submitStatus === 'submission.failedProcessingBuild' || submitStatus === 'job.submitting' || submitStatus === 'job.registering' || submitStatus === 'job.submitFailed') {
    return of(1).pipe(
      delay(5000)
    );
  }
...
const submitApplicationsEpic = (action, state) => action.pipe(
  ...
  mergeMap(([a, s]) => {
    ...
    return from(artifacts).pipe(
      ...
      map(() => (getSubmitStatus(submitInfo.id))),
      catchError(error => of(handleError(a, error)))
    );
  }),
);
      ...
      mergeMap(bundle => StreamsRestUtils.artifact.uploadApplicationBundleToInstance(s, bundle.bundlePath).pipe(
        mergeMap(uploadBundleResponse => {
          ...
          return StreamsRestUtils.asyncSubmissions.submitJobAsync(s, submitBundleId, {}).pipe(
            ...
            map((submitResponse) => (getSubmitStatus(ResponseSelector.getSubmitInfo(submitResponse).id)))
          );
        })
      )),
      catchError(error => of(handleError(a, error)))
