...
import {
  ...
  queueAction,
  newBuild,
  submitApplicationsFromBundleFilesAsync,
  setToolkitsCacheDir,
...
export default {
  ...
  shouldShowMenuSpl(event) {
    if (event.path[0].className !== 'name icon icon-file-text') {
  ...
  extractSab(bundlePath) {
    ...
    extract(bundlePath, { dir: extractedPath }, (err) => {
      ...
      fs.createReadStream(`${extractedPath}${path.sep}tar${path.sep}bundle.tar`).pipe(tar.extract(`${extractedPath}${path.sep}tar${path.sep}bundle`, {
        finish: () => {
          this.getSubmissionTimeParams(bundlePath, extractedPath);
        }
      }));
    });
    return bundlePath;
  ...
  getSubmissionTimeParams(bundlePath, extractedPath) {
    ...
    console.log(doc);
    console.log(xml);
    const submissionTimeParams = doc.childNamed('tuple');
    console.log(submissionTimeParams);
  ...
  submitV5() {
    ...
    const submitAction = submitApplicationsFromBundleFilesAsync(bundles);
    if (!StateSelector.hasAuthenticatedToStreamsInstance(getStore().getState())) {
  ...
  submitV4() {
    const selectedFilePath = this.treeView.selectedPaths()[0];
    this.extractSab(selectedFilePath);
