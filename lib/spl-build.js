...
export default {
  ...
  registerCommands() {
    this.subscriptions.add(
      atom.commands.add('atom-workspace', 'spl-build:build-submit', {
        ...
        didDispatch: () => this.buildAndSubmit(StreamsUtils.BUILD_ACTION.SUBMIT)
      }),
  ...
  buildAndSubmit(action) {
    if (this.apiVersion === CONF_API_VERSION_V5) {
      this.handleV5Action(() => this.buildAndSubmitV5());
    } else {
      this.buildAppV4(action);
    }
  },
  ...
  buildAndSubmitV5() {
    ...
    if (mainComposites.length === 1) {
      ...
      const newBuildAndSubmitAction = buildAndSubmit(
        {
          appRoot,
          toolkitRootPath,
          fqn,
      ...
      if (!StateSelector.hasAuthenticatedToStreamsInstance(getStore().getState())) {
        getStore().dispatch(queueAction(newBuildAndSubmitAction));
        this.showAuthPanel();
      } else {
        getStore().dispatch(newBuildAndSubmitAction);
      }
    } else {
      this.appRoot = appRoot;
      this.namespace = namespace;
