...
function buildStatusUpdate(action, state) {
  ...
  // const buildObj = getBuild(state, buildId);
  // const buildStatus = getBuildStatus(state, buildId);
...
function jobSubmitted(state, submitInfo, buildId) {
  ...
  if (submitInfo.status === 'running') {
    messageHandler.handleSuccess(
      ...
      {
        detail: 'To monitor or manage the job, use the ICP4D manageJobs page or the Streams console',
        notificationAutoDismiss: false,
        notificationButtons: [
          {
