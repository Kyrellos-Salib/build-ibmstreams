...
function parseToolkitVersions(dependencies) {
  for (let i = 0; i < dependencies.length; i += 1) {
    const vr = dependencies[i].version.split(',');
    let range = '';
    if (vr.length === 2) {
      range = '';
      if (vr[0].charAt(0) === '[') {
    ...
    try {
      ...
      if (dependenciesNode) {
        ...
        if (dependencyToolkitsNodes) {
          ...
          parseToolkitVersions(dependencies);
          const newestLocalToolkits = StreamsToolkitsUtils.filterNewestToolkits(allToolkits).filter(tk => tk.isLocal);
          const toolkitsToInclude = _.intersectionWith(newestLocalToolkits, dependencies, (tk, dependency) => tk.name === dependency.name && semver.satisfies(tk.version, dependency.version));
