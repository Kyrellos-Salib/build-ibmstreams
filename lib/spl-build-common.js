    ...
    return new Promise(resolve => {
      ...
      extract(bundlePath, { dir: extractedPath }, (err) => {
        ...
        fs.createReadStream(`${extractedPath}${path.sep}tar${path.sep}bundle.tar`).pipe(tar.extract(`${extractedPath}${path.sep}tar${path.sep}bundle`, {
          finish: () => {
            resolve(this.getSubmissionTimeParams(bundlePath, extractedPath, action));
          }
        }));
      });
    });
        ...
        callbackFn: () => {
          ...
          if (submissionObservableInput.artifacts) {
            this.downloadBundlesObservable(submissionObservableInput).pipe(
              ...
              complete => {
                this.extractSab(`${outputDir}${path.sep}temp${path.sep}${submissionObservableInput.artifacts[0].name}`, {
            ...
            this.extractSab(submissionObservableInput.filename, {
              type: 'V4', submissionObservableFunc, submissionObservableInput, submitFunc
            }).then(panel => {
              if (panel) {
                panel.show();
