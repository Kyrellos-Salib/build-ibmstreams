        ...
        callbackFn: () => {
          ...
          if (submissionObservableInput.artifacts) {
            this.downloadBundlesObservable(submissionObservableInput).pipe(
              ...
              complete => {
                this.extractSab(`${outputDir}${path.sep}temp${path.sep}${submissionObservableInput.artifacts[0].name}`, submissionObservableFunc, submissionObservableInput, submitFunc).then(panel => {
                  if (panel) {
                    panel.show();
                  }
