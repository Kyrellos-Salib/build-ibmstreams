...
export class SplBuilder {
  ...
  buildAndSubmitJob(input) {
    ...
    submitSourceAndWaitForBuild.pipe(
      ...
      mergeMap(downloadResult => this.performBundleDownloads(downloadResult, input, `${path.dirname(input.filename)}${path.sep}output${path.sep}temp`)),
    ).subscribe(
      next => { },
      err => {
        let errorNotification = null;
  ...
  extractSab(bundlePath, submissionObservableFunc, submissionObservableInput, submitFunc) {
    return new Promise(resolve => {
      ...
      extract(bundlePath, { dir: extractedPath }, (err) => {
        ...
        fs.createReadStream(`${extractedPath}${path.sep}tar${path.sep}bundle.tar`).pipe(tar.extract(`${extractedPath}${path.sep}tar${path.sep}bundle`, {
          finish: () => {
            resolve(this.getSubmissionTimeParams(bundlePath, extractedPath, submissionObservableFunc, submissionObservableInput, submitFunc));
          }
        }));
      });
    });
  ...
  getSubmissionTimeParams(bundlePath, extractedPath, submissionObservableFunc, submissionObservableInput, submitFunc) {
    if (fs.existsSync(`${extractedPath}${path.sep}tar${path.sep}bundle`)) {
      ...
      if (submissionTimeValues) {
        ...
        }, submissionTimeValues.childrenNamed('submissionTimeValue'), submissionObservableFunc, submissionObservableInput, submitFunc);
    ...
    const dialogButtons = [
      {
        ...
        callbackFn: () => {
          ...
          if (submissionObservableInput.artifacts) {
            this.this.extractSab(`${outputDir}${path.sep}temp${path.sep}${submissionObservableInput.artifacts[0].name}`, submissionObservableFunc, submissionObservableInput, submitFunc).then(panel => {
              if (panel) {
                panel.show();
              }
            });
            fse.removeSync(`${outputDir}${path.sep}temp${path.sep}${submissionObservableInput.artifacts[0].name}`);
          } else {
            this.extractSab(submissionObservableInput.filename, submissionObservableFunc, submissionObservableInput, submitFunc).then(panel => {
              if (panel) {
                panel.show();
              }
            });
