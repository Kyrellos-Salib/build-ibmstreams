			...
			switchMap(artifacts => this.getConsoleUrlObservable().pipe(
				...
				map(consoleResult => {
					...
					if (consoleResponse.body["streams_console"] && consoleResponse.body["id"]) {
						...
						const dialogMessage = "Use your IBM Cloud Streaming Analytics service console for job submission";
						const dialogDetail = `If your job(s) require submission time configuration, copy the output path and use the Streams Console. Open the "Submit Job" dialog by clicking on the 'play' button in the header of the console. Then click on the "Browse" button and select an application bundle in the following folder:\n\n${outputDir}\n\nFrom there, you can configure the job for submission. If you would like to submit multiple bundles, repeat as necessary.\n\nIf your job(s) do not require any submission time configuration, you can submit with default configuration.`;
						const dialogButtons = [
							{
								label: "Copy output path to clipboard",
								callbackFn: () => {
									ncp.copy(outputDir);
									this.downloadBundlesObservable(artifacts).pipe(
										map(downloadOutput => ( [ artifacts, downloadOutput ])),
										mergeMap(downloadResult => this.performBundleDownloads(downloadResult, input)),
									).subscribe(console.log);
								}
							},
							{
								label: "Copy output path to clipboard and open the Streaming Analytics console",
								callbackFn: () => {
									ncp.copy(outputDir);
							...
							{
								label: "Submit job(s) with default configuration",
								callbackFn: () => {
									this.submitAppObservable(artifacts).pipe(
										tap(a => console.log("inside submit app callback observable",a))
									).subscribe(console.log);
								}
							},
							{ label: "Close",
