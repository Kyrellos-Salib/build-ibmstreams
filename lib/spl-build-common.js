			...
			switchMap(artifacts => this.getConsoleUrlObservable().pipe(
				...
				map(consoleResult => {
					...
					if (consoleResponse.body["streams_console"] && consoleResponse.body["id"]) {
						...
						const dialogDetail = `To submit your job, open the "Submit Job" dialog by pressing on the play button in the header of the Streaming Analytics console. Then, click on the "Browse" button and select a bundle in the following folder:\n\n${outputDir}\n\nFrom there, you may either configure the job submission or submit the job with the default configuration settings.\n\nIf you would like to submit multiple bundles, repeat as necessary.`;
						const dialogButtons = [
							{
								...
								callbackFn: () => {
									...
									this.downloadBundlesObservable(artifacts).pipe(
										mergeMap(a => this.performBundleDownloads(a, input))
									).subscribe(console.log);
								}
							},
							{
								label: "Copy output path to clipboard and open Streaming Analytics console",
								callbackFn: () => {ncp.copy(outputDir); this.openUrlHandler(consoleUrl)}
							},
							{
								label: "Submit job with default configuration",
								callbackFn: () => {
									this.submitAppObservable(consoleResult).subscribe(console.log);
								}
							},
							{ label: "Close",
								callbackFn: null
