...
export class SplBuilder {
	...
	async buildSourceArchive(appRoot, toolkitRootPath, fqn) {
		...
		try {
			...
			if (toolkitPaths) {
				...
				tkPathString = `:..${path.sep}toolkits`;
				makefilePath = `${newRoot}${path.sep}`;
			...
			if (!fs.existsSync(`${appRoot}${path.sep}Makefile`) && !fs.existsSync(`${appRoot}${path.sep}makefile`)) {
				const makeCmd = `main:\n\tsc -M ${fqn} -t $$STREAMS_INSTALL${path.sep}toolkits${tkPathString}`;
	...
	static getApplicationRoot(rootDirArray, filePath) {
		if (typeof(filePath) === "string" && Array.isArray(rootDirArray)) {
			...
			const noMatchingFiles = dir => !fs.existsSync(`${dir}${path.sep}info.xml`) && !fs.existsSync(`${dir}${path.sep}toolkit.xml`) && !fs.existsSync(`${dir}/Makefile`) && !fs.existsSync(`${dir}${path.sep}makefile`);
